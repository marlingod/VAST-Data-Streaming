...
def handler(ctx, event):
    ...
    try:
        ...
        # The event could be a direct transaction or an S3 notification
        # For S3 triggers, we need to fetch the actual data
        if isinstance(event_data, dict) and "Records" in event_data:
            # S3 event notification — the transaction data is in the blob expansion table
            ctx.logger.info("S3 event received — processing via blob expansion table")
            # In this case, we'd query the blob expansion table for new records
            # For now, return early as the scoring happens on direct consumption
